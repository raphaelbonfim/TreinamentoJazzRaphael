...
namespace TreinamentoJazzRaphael.Tests
{
    public abstract class TreinamentoJazzRaphaelTestBase : AbpIntegratedTestBase<TreinamentoJazzRaphaelTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected TreinamentoJazzRaphaelTestBase()
        {
        ...
        protected void UsingDbContext(Action<TreinamentoJazzRaphaelDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<TreinamentoJazzRaphaelDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<TreinamentoJazzRaphaelDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<TreinamentoJazzRaphaelDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<TreinamentoJazzRaphaelDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<TreinamentoJazzRaphaelDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<TreinamentoJazzRaphaelDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<TreinamentoJazzRaphaelDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<TreinamentoJazzRaphaelDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<TreinamentoJazzRaphaelDbContext, Task<T>> func)
        {
            T result;
